...
async function loadProfile() {
  ...
  try {
    const [profile, walletResult, pointResult, memberResult, cashInResult] = await Promise.all([
    ...
    for (const result of [walletResult, pointResult, memberResult, cashInResult]) {
    ...
    document.getElementById("cashInRequests").innerHTML =
      cashInResult.data.length ? cashInResult.data.map(row => `
        <tr>
...
document.getElementById("cashInForm").addEventListener("submit", async event => {
  ...
  showMessage(profileMessage, "Creating your pending cash-in request...");
  ...
  document.getElementById("paymentAmount").textContent = money(amount).replace("$", "₱");
...
document.getElementById("referenceForm").addEventListener("submit", async event => {
  ...
  showMessage(profileMessage, "Submitting your reference number...");
  ...
  showMessage(profileMessage, "Reference submitted. Your request is now waiting for admin verification.", true);
  await loadProfile();
});
